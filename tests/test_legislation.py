...
def test_legislation():
    ...
    law = get_total_legislation_introduced_by_date_range(
        begin_date, end_date, 0, 0, True)
    
    # only supposed to return non-empty results
    # when prefiled bills exist, just before session starts
    prelaw = get_prefiled_legislation()
    prelawinfo = get_pre_filed_legislation_info()
    
    # bug: results are not unpacked correctly!
    features = get_legislative_bill_list_feature_data()
